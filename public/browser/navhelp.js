...
// Pass board object, html table
Board.prototype.restore=function(boardObject, tempTable){
  
  
  console.log('RESTORING SAVED DATA');
  this.height = boardObject.height;
  this.width = boardObject.width;
  this.start = boardObject.start;
  this.target = boardObject.target;
  this.object = boardObject.object;
  this.boardArray = boardObject.boardArray;
  this.nodes = boardObject.nodes;
  this.nodesToAnimate = boardObject.nodesToAnimate;
  this.objectNodesToAnimate = boardObject.objectNodesToAnimate;
  this.shortestPathNodesToAnimate = boardObject.shortestPathNodesToAnimate;
  this.objectShortestPathNodesToAnimate = boardObject.objectShortestPathNodesToAnimate;
  this.wallsToAnimate = boardObject.wallsToAnimate;
  this.mouseDown = boardObject.mouseDown;
  this.pressedNodeStatus = boardObject.pressedNodeStatus;
  this.previouslyPressedNodeStatus = boardObject.previouslyPressedNodeStatus;
  this.previouslySwitchedNode = boardObject.previouslySwitchedNode;
  this.previouslySwitchedNodeWeight = boardObject.previouslySwitchedNodeWeight;
  this.keyDown = boardObject.keyDown;
  this.algoDone = boardObject.algoDone;
  this.currentAlgorithm = boardObject.currentAlgorithm;
  this.currentHeuristic = boardObject.currentHeuristic;
  this.numberOfObjects = boardObject.numberOfObjects;
  this.isObject = boardObject.isObject;
  this.buttonsOn = false;
  this.speed = boardObject.speed;
  
  // newBoard1 = JSON.parse(JSON.stringify(boardObject));
  // console.log("toggle se pehle");
  // console.log(this.nodes);

  
  this.recreateGrid(tempTable); 
  this.addEventListeners();
  this.toggleButtons();
}


Board.prototype.recreateGrid = function(tempTable) {
  let board = document.getElementById("board");
  board.innerHTML = tempTable;
};
...
Board.prototype.reset = function(objectNotTransparent) {
  ...
  document.getElementById(this.start).className = "startTransparen movement";
...
Board.prototype.startMovement = function() {
  // document.getElementById(this.start).classList.add("movement");
  // document.getElementsByClassName("startTransparent");
  // console.log(document.getElementById(this.start).classList);
  // document.getElementsByClassName("startTransparent").className = 'movement';
  // var temp = document.getElementsByClassName("startTransparent");
  // console.log(temp[0]);
  
  let userLocation = new Movement();
  userLocation.update(this.start);
  console.log("done");
  // docoument.getElementById
}

Board.prototype.toggleButtons = function() {
  // console.log("toggle ke andar");
  // console.log(this.nodes);
  ...
  if (!this.buttonsOn) {
    ...
    document.getElementById("startButtonClearBoard").onclick = () => {
      ...
      let height = Math.floor(($("#board-container").height()) / inp.value);
      let width = Math.floor($("#board-container").width() / inp.value);
...
let height = Math.floor(($("#board-container").height()) / 25);
let width = Math.floor($("#board-container").width() / 25);
let newBoard = new Board(height, width);
newBoard.initialise();

window.newBoard=newBoard;
// console.log(newBoard.nodes);
...
function myfunc(){
    ...
    height = Math.floor(($("#board-container").height()) / inp.value);
    width = Math.floor($("#board-container").width() / inp.value);
    newBoard = new Board(height, width);
    newBoard.initialise();
    window.newBoard=newBoard
    var all = document.getElementsByTagName('td');
    for (var j = 0; j < all.length; j++) {
        all[j].style.height = inp.value+'px';
        all[j].style.width = inp.value+'px';
    }
}
